...
class _TrendingShopsScreenState extends State<TrendingShopsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        final all = Get.find<AppController>().boutiques.toList();
        var trending = all.where((b) => b.noteMoyenne >= 4.5).toList();
        if (trending.isEmpty) trending = List.of(all);

        if (_selectedFilter != 'Tout') {
          final key = _selectedFilter.toLowerCase();
          final byCat = trending.where((b) {
            final cats = b.categories ?? [];
            for (final c in cats) {
              final nom =
                  (c is Map ? c['nom']?.toString() : c.toString()) ?? '';
              if (nom.toLowerCase().contains(key)) return true;
            }
            return false;
          }).toList();
          if (byCat.isNotEmpty) trending = byCat;
        }
                        ...
                        return AnimationConfiguration.staggeredGrid(
                          ...
                          child: ScaleAnimation(
                            child: FadeInAnimation(
                              child:
                                  ShopCarouselCard(boutique: boutique),
                            ),
                          ),
                        );
                      },
