...
class _OrderCheckoutScreenState extends State<OrderCheckoutScreen> {
  ...
  Future<bool> _showConfirmationDialog(Product product, String priceMain) async {
    final modeLabel = _mode == 'livraison' ? 'Livraison à domicile' : 'Retrait en boutique';
    final payLabel = _paymentMode == 'livraison' ? 'À la livraison / retrait' : 'Entente en messagerie';
              ...
              Padding(
                ...
                child: Column(
                  children: [
                        ...
                        ClipRRect(
                          ...
                          child: CachedNetworkImage(
                            ...
                            errorWidget: (_, __, ___) => Container(
                              ...
                              child: Icon(Icons.image_not_supported_outlined, color: AppTheme.mutedForeground, size: 22),
                            ),
                          ),
                        ),
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                ...
                                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.foreground),
                              ),
                              const SizedBox(height: 4),
                              Text(
                                '$priceMain FCFA',
                    ...
                    // Lignes de récap
                    _confirmRow(Icons.local_shipping_outlined, 'Livraison', modeLabel),
                    const SizedBox(height: 8),
                    _confirmRow(Icons.credit_card_outlined, 'Paiement', payLabel),
                    const SizedBox(height: 8),
                    _confirmRow(Icons.phone_outlined, 'Téléphone', _formattedPhone),
                    if (_mode == 'livraison' && _addressCtrl.text.isNotEmpty) ...[
                      const SizedBox(height: 8),
                      _confirmRow(Icons.location_on_outlined, 'Adresse', _addressCtrl.text),
                    ],
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: const Text('Annuler', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      ...
                      child: ElevatedButton(
                        ...
                        child: const Text('Confirmer', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _confirmRow(IconData icon, String label, String value) {
    return Row(
      ...
      children: [
        ...
        Expanded(
          flex: 2,
          child: Text(label, style: TextStyle(fontSize: 12, color: AppTheme.mutedForeground)),
        ),
        Expanded(
          flex: 3,
          child: Text(
  ...
  Future<void> _requestGpsPosition() async {
    ...
    try {
      ...
      setState(() { _deliveryLat = position.latitude; _deliveryLon = position.longitude; _gpsLoading = false; });
      _mapController.move(LatLng(position.latitude, position.longitude), 15.0);
      _reverseGeocodeAndFillAddress(position.latitude, position.longitude);
      AppToasts.success(context, 'Localisation récupérée', 'Votre position a été ajoutée.');
    } catch (e) {
  ...
  Future<void> _onMapTap(LatLng point) async {
    setState(() { _deliveryLat = point.latitude; _deliveryLon = point.longitude; });
  ...
  Future<void> _reverseGeocodeAndFillAddress(double lat, double lon) async {
    ...
    if (data != null && mounted) {
      String formatted = '';
      if (data['ville'] != null && data['ville']!.isNotEmpty) formatted += data['ville']!;
  ...
  Product? _getProduct(dynamic productId) {
    try {
      ...
      final p = ctrl.products.firstWhereOrNull((p) => p.id.toString() == productId.toString());
      if (p != null) return p;
    } catch (_) {}
  ...
  Widget _recapRow(R r, IconData icon, String label, String value, {Color? valueColor, bool bold = false}) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            flex: 2,
            child: Text(label, style: TextStyle(fontSize: r.fs(13), color: AppTheme.mutedForeground)),
          ),
          Expanded(
            flex: 3,
            child: Text(
  ...
  Widget _recapDivider() => Divider(height: 1, thickness: 0.5, color: AppTheme.border);
  ...
  Widget build(BuildContext context) {
    ...
    if (_confirmed) {
      ...
      final dateStr = '${orderDate.day.toString().padLeft(2, '0')}/${orderDate.month.toString().padLeft(2, '0')}/${orderDate.year}  ${orderDate.hour.toString().padLeft(2, '0')}:${orderDate.minute.toString().padLeft(2, '0')}';
      final modeLabel = _mode == 'livraison' ? 'Livraison à domicile' : 'Retrait en boutique';
      final modeIcon = _mode == 'livraison' ? Icons.delivery_dining_outlined : Icons.storefront_outlined;
      final payLabel = _paymentMode == 'livraison' ? 'À la livraison/retrait' : 'Entente en messagerie';
      ...
      return Scaffold(
        ...
        body: SafeArea(
          child: SingleChildScrollView(
            padding: EdgeInsets.symmetric(horizontal: r.hPad, vertical: r.s(24)),
            child: Column(
              ...
              children: [
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(Icons.check_circle_rounded, size: r.s(40), color: AppTheme.primary),
                ...
                Text(
                  ...
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: r.fs(13), color: AppTheme.mutedForeground, height: 1.5),
                ),
                  ...
                  child: Column(
                    children: [
                            ...
                            ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                ...
                                errorWidget: (_, __, ___) => Container(
                                  ...
                                  color: AppTheme.muted,
                                  child: Icon(Icons.image_not_supported_outlined, color: AppTheme.mutedForeground),
                                ),
                              ),
                            ),
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Row(
                                          mainAxisSize: MainAxisSize.min,
                                          children: [
                                            Icon(Icons.hourglass_top_outlined, size: 12, color: Colors.orange.shade700),
                                            const SizedBox(width: 4),
                                            Text(
                                              'En attente',
                                              style: TextStyle(fontSize: r.fs(11), fontWeight: FontWeight.w600, color: Colors.orange.shade700),
                                            ),
                                          ],
                                        ),
                                      ),
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            _recapRow(r, Icons.tag_outlined, 'N° Commande', '#$orderId', bold: true),
                            _recapDivider(),
                            _recapRow(r, Icons.calendar_today_outlined, 'Date', dateStr),
                            _recapDivider(),
                            _recapRow(r, Icons.shopping_bag_outlined, 'Produit', product.title),
                            _recapDivider(),
                            _recapRow(r, Icons.numbers_outlined, 'Quantité', '1'),
                            _recapDivider(),
                            _recapRow(r, Icons.payments_outlined, 'Prix total', '$priceMain FCFA', valueColor: AppTheme.primary, bold: true),
                            _recapDivider(),
                            _recapRowIcon(r, modeIcon, 'Livraison', modeLabel),
                            if (_mode == 'livraison' && _addressCtrl.text.isNotEmpty) ...[
                              _recapDivider(),
                              _recapRow(r, Icons.location_on_outlined, 'Adresse', _addressCtrl.text),
                            ],
                            _recapDivider(),
                            _recapRow(r, Icons.credit_card_outlined, 'Paiement', payLabel),
                            _recapDivider(),
                            _recapRow(r, Icons.phone_outlined, 'Téléphone', _formattedPhone),
                            if (_noteCtrl.text.isNotEmpty) ...[
                              _recapDivider(),
                              _recapRow(r, Icons.notes_outlined, 'Note', _noteCtrl.text),
                            ],
                          ],
                        ),
                      ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      foregroundColor: Colors.white,
                      elevation: 0,
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(r.rad(18))),
                    ),
                    icon: const Icon(Icons.receipt_long_outlined),
                    label: Text('Voir mes commandes', style: TextStyle(fontSize: r.fs(15), fontWeight: FontWeight.w700)),
                  ),
                ),
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    label: Text('Retour à l\'accueil', style: TextStyle(fontSize: r.fs(14), fontWeight: FontWeight.w600)),
                  ),
                ),
          ...
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(r.rad(12)),
                border: Border.all(
                  color: _phoneError != null ? AppTheme.destructive : AppTheme.border,
                ),
              ),
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        _phoneError!,
                        style: TextStyle(fontSize: r.fs(12), color: AppTheme.destructive),
                      ),
                    ),
                  ],
                ),
                ...
                child: Stack(
                  children: [
                    FlutterMap(
                      ...
                      options: MapOptions(
                        initialCenter: _deliveryLat != null && _deliveryLon != null
              ...
              Container(
                ...
                child: Column(
                  children: [
                            ...
                            children: [
                              Text(
                                product?.boutiqueObj?.nom ??
                                    (product != null ? (getSellerById(product.sellerId.toString())?.shopName ?? 'Boutique du vendeur') : 'Boutique du vendeur'),
                                style: TextStyle(
                                    fontSize: r.fs(14),
                                    fontWeight: FontWeight.w700,
                                    color: AppTheme.foreground),
                              ...
                              Text(
                                product?.boutiqueObj?.adresse ??
                                    ...
                                    (product != null ? (getSellerById(product.sellerId.toString())?.location ?? 'Lomé, Togo') : 'Lomé, Togo'),
                                style: TextStyle(
                                    fontSize: r.fs(12),
                                    color: AppTheme.mutedForeground),
                              ),
                    ...
                    if (product?.boutiqueObj != null && product!.boutiqueObj!.latitude != 0 && product.boutiqueObj!.longitude != 0)
                      Container(
                        height: 140,
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(r.rad(12)),
                        ...
                        child: FlutterMap(
                          options: MapOptions(
                            initialCenter: LatLng(product.boutiqueObj!.latitude, product.boutiqueObj!.longitude),
                            initialZoom: 15.0,
                          ...
                          children: [
                            ...
                            MarkerLayer(
                              markers: [
                                Marker(
                                  point: LatLng(product.boutiqueObj!.latitude, product.boutiqueObj!.longitude),
                                  width: 40, height: 40,
                                  child: const Icon(Icons.storefront, color: Colors.deepOrange, size: 36),
                                ),
                              ],
                            ),
                          ],
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () async {
                          ...
                          Uri? uri;
                          if (boutique != null && boutique.latitude != 0 && boutique.longitude != 0) {
                            uri = Uri.parse('https://www.google.com/maps/search/?api=1&query=${boutique.latitude},${boutique.longitude}');
                          } else {
                            // Fallback to mock seller
                            final seller = getSellerById(product?.sellerId?.toString() ?? '');
                            if (seller != null && seller.location.isNotEmpty) {
                              uri = Uri.parse('https://www.google.com/maps/search/?api=1&query=${Uri.encodeComponent(seller.location)}');
                            } else {
                              AppToasts.info(context, 'Information', 'La position du vendeur n\'est pas renseignée.');
                            }
                          }
                          if (uri != null && await canLaunchUrl(uri)) {
                            await launchUrl(uri, mode: LaunchMode.externalApplication);
                          } else if (uri == null) {
                            // Already handled by AppToasts.info above
                          } else {
                            AppToasts.error(context, 'Erreur', 'Impossible d\'ouvrir Google Maps.');
                          }
                        },
            ...
            SizedBox(height: r.s(22)),
            Row(children: [
              Icon(Icons.credit_card_outlined, size: r.s(18), color: AppTheme.primary),
              SizedBox(width: r.s(7)),
            ...
            _OrderModeOption(
              ...
              title: _mode == 'livraison' ? 'Paiement à la livraison' : 'Paiement au moment du retrait',
          ...
          children: [
            ...
            GestureDetector(
              onTap: _isLoading ? null : () async {
                ...
                final userConfirmed = await _showConfirmationDialog(product, priceMain);
                if (!userConfirmed) return;
                setState(() => _isLoading = true);
                try {
                  // 1. Créer la commande sur le backend
                  final orderResp = await _orderService.createOrder(
                    productId: int.tryParse(product.id?.toString() ?? '0') ?? 1,
                    quantity: 1,
                  ...
                  _orderResult = orderResp['order'] as Map<String, dynamic>?;
                  ...
                  if (senderId.isNotEmpty && sellerId.isNotEmpty) {
                    ...
                    final orderData = {
                      ...
                      'quantity': '1',
                      'total': '$priceMain FCFA',
                      'mode': modeLabel,
                      'address': (_mode == 'livraison' && _addressCtrl.text.isNotEmpty) ? _addressCtrl.text : '',
                      'payment': payLabel,
                      'phone': _formattedPhone,
                      'note': _noteCtrl.text,
                      'status': 'En attente',
                    ...
                    final buyerName = currentUser?.nom ?? currentUser?.email ?? 'Acheteur';
                    final buyerAvatar = currentUser?.avatarUrl ?? '';
                    final sellerName = product.boutiqueObj?.nom ?? product.userObj?.nom ?? 'Vendeur';
                    final sellerAvatar = product.boutiqueObj?.logoUrl ?? product.userObj?.avatarUrl ?? '';
                  ...
                  AppToasts.success(context, 'Succès', 'Votre commande a été transmise au vendeur.');
                  setState(() { _isLoading = false; _confirmed = true; });
                  _startCountdown();
                } catch (e) {
                  setState(() => _isLoading = false);
                  AppToasts.error(context, 'Échec de la commande', e.toString());
                }
              },
              child: Container(
                width: double.infinity,
